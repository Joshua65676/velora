...
const Hero: React.FC = () => {
  const [activeBg, setActiveBg] = useState(HeroList[0].bg);

  return (
    <main className="xl:w-374 md:w-370 w-full xl:m-2">
      <main
        className="bg-center bg-cover bg-origin-padding bg-no-repeat container xl:mx-auto xl:max-w-370 xl:h-screen md:max-w-370 max-w-full rounded-[10px] transition-all duration-500"
      ...
      >
        <section className="container mx-auto max-w-7xl w-full py-35 px-6 flex flex-col xl:items-center gap-47">
          ...
          <Hero2 activeBg={activeBg} setActiveBg={setActiveBg} />
        </section>
      </main>
    </main>
  );
